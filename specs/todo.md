...
- [x] **T0.2a** Postgres impl — cron methods (`cron_upsert` via `ON CONFLICT`, `cron_load_due`, `cron_load_by_id`, `cron_mark_fired`, `cron_job_active`, `cron_cancel_job`, `cron_delete`, `cron_advisory_lock/unlock`). 8 live tests. *(Advisory lock now holds on a dedicated persistent connection (T0.3) — real cross-node mutual exclusion.)* Added missing `jitter_ms` column to the schema.
